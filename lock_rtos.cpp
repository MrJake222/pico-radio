...
static inline bool rtos_running() {
    // after scheduler is started, init function should be finished
...
// code is run on RTOS core
static inline bool rtos_core() {
    return get_core_num() == FREERTOS_CORE;
}

static inline uint get_lock_bit(uint lock_num) {
    // map higher-numbered locks to 0-15
    // make bitfield (16-bit)
    return 1u << (lock_num & 0x0F);
}

static inline TickType_t until_to_ticks(absolute_time_t until) {
    return us_to_ms(absolute_time_diff_us(get_absolute_time(), until)) / portTICK_PERIOD_MS;
}

static void rtos_fifo_cb(void* arg, uint32_t data) {
    // run from ISR
...
int freertos_get_caller_owner_id() {
...
static void freertos_wait_internal(struct lock_core* lock, TickType_t ticks_to_wait) {
    xEventGroupWaitBits(
            event_group_h,
...
void freertos_wait(struct lock_core* lock) {
    if (!rtos_core() || !rtos_running()) {
        __wfe();
        return;
    }
...
bool freertos_wait_until(struct lock_core* lock, absolute_time_t until) {
    /* Waiting function
     * wait on one of:
...
void freertos_notify(struct lock_core* lock) {
    /* Notifying function
     * notify all of:
    ...
    if (!rtos_core()) {
        ...
        fifo_send_with_data(LOCK_RTOS, spin_lock_get_num(lock->spin_lock));
        return;
    }
...
void freertos_delay(absolute_time_t until) {
    if (!rtos_running())
        return;
